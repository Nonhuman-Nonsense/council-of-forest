...
describe('PronunciationUtils', () => {
    ...
    it('should spell out meeting numbers for English TTS', () => {
        const result = PronunciationUtils.processText(
            'This concludes Council of Forest meeting #1020.',
            'en',
            { includeIpa: false },
        );
        expect(result.processedText).toBe(
            'This concludes Council of Forest meeting number 1020.',
        );
    ...
    it('should spell out meeting numbers for Swedish TTS', () => {
        const result = PronunciationUtils.processText(
            'Detta avslutar Skogsrådets möte #1020.',
            'sv',
            { includeIpa: false },
        );
        expect(result.processedText).toBe(
            'Detta avslutar Skogsrådets möte nummer 1020.',
        );
