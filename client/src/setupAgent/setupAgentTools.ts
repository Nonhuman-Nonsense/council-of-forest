...
export function createSetupAgentTools({
  ...
  const tools: RealtimeTool[] = [
    ...
    },
    {
      ...
      name: "select_topic",
      description:
        "Highlight a topic in the UI by title and return notes on what is at stake in it, to speak from (not to recite — the visitor can already read what the topic is). Does NOT advance to being selection — call confirm_topic when the visitor is ready to proceed.",
      parameters: {
        type: "object",
